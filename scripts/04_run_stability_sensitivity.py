...
def main() -> None:
    seeds = [0, 1, 2, 3, 4]
    mc_grid = [1, 5]
    rho_max_grid = [0.05, 0.02]

    results: list[dict] = []

    print("=" * 80)
    print("Running Experiment 2: stability sensitivity at N=100")
    print("=" * 80)

    for n_mc_samples in mc_grid:
        for rho_max in rho_max_grid:
            print(f"\n--- condition: n_mc_samples={n_mc_samples}, rho_max={rho_max:.2f} ---")
    ...
    output_dir = Path("outputs") / "experiments" / "stability_sensitivity"
    ...
    if len(df_ok) > 0:
        summary = (
            df_ok.groupby(["n_mc_samples", "rho_max"])[
                ["rho_error", "W_error", "rmse", "runtime_sec", "final_elbo"]
            ]
            .mean()
            .reset_index()
        )
    ...
    failure_summary = (
        df.groupby(["n_mc_samples", "rho_max"])["status"]
